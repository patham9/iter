...
DESCRIPTION = "Contradict a memory item in chroma_db by linking an episode and applying NAL truth revision with negative evidence. Item_id must be a UUID from chroma_query results."
...
def run(item_id, episode_time):
    """
    item_id: str - UUID of the memory item to contradict
    episode_time: str - timestamp of the contradicting episode
    ...
    if episode_time not in linked_episodes:
        ...
        # Apply truth revision with negative evidence: (0.0, 0.5)
        s2, c2 = 0.0, 0.5
        ...
        return f"CONTRADICT-SUCCESS: item {item_id} stv ({s1},{c1}) -> ({new_s},{new_c}), episode {episode_time} linked"
    return "CONTRADICT-FAIL: episode already in episode list"
